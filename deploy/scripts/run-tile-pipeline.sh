...
# ---------------------------------------------------------------------------
# Idempotent, side-effect-safe retention for the EFS region dir. Keeps the
# newest ${KEEP_VERSIONS} published v<TAG> dirs (the tar lives INSIDE each, so
# pruning the dir reclaims its .tar too) and removes leftover *.partial staging
# dirs from crashed runs — each *.partial is a full-size copy that the version
# prune deliberately ignores, so it must be reclaimed here or it leaks forever.
#
...
# ---------------------------------------------------------------------------
_efs_prune() {
    local efs_region_dir="$1"
    [[ -d "${efs_region_dir}" ]] || return 0
    ...
    done < <(find "${efs_region_dir}" -maxdepth 1 -type d -name "v*.partial" -print0 2>/dev/null)

    # Prune published versions — keep newest N. RUN_ID is a zero-padded
    # %Y%m%d-%H%M%S stamp, so lexical sort == chronological order.
    local versions
    mapfile -t versions < <(
        find "${efs_region_dir}" -maxdepth 1 -type d -name "v[0-9]*" ! -name "*.partial" 2>/dev/null \
        | sort
    )
...
phase_efs_sync() {
    ...
    _efs_prune "${efs_region_dir}" || log_warn "EFS pre-sync prune failed (non-fatal)"
    ...
    _efs_prune "${efs_region_dir}" || log_warn "EFS post-swap prune failed (non-fatal)"
}
