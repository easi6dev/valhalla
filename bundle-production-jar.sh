 #!/bin/bash                                                                                                                                                                                                                       
  ...
  JAR_FILE=$(ls build/libs/valhalla-jni-*.jar 2>/dev/null | head -1)
  if [ -n "$JAR_FILE" ]; then
  ...
  ./gradlew test --tests "SingaporeRideHaulingTest" -x buildNative
  ...
  if [ $? -eq 0 ]; then
      ...
      echo ""
      echo "📝 Usage:"
      echo "   java -jar valhalla-jni-*.jar"
      echo ""
  else
      echo ""
      echo "╔═══════════════════════════════════════════════════════════╗"
